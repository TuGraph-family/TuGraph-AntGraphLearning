...
from agl.python.subgraph.pyg_inputs import TorchEgoBatchData
...
class GeniePathLayer(torch.nn.Module):
    ...
    def forward(self, x, edge_index, h, c, size=None):
        ...
        x = x[None, :]
        x, (h, c) = self.depth(x, h, c) #self.depth(x, h[:, :x.shape[1]], c[:, :x.shape[1]])
        return x[0], (h, c)
...
class GeniepathEncoder(torch.nn.Module):
    ...
    def forward(self, subgraph: TorchEgoBatchData):
