...
class KCANEncoder(torch.nn.Module):
    def __init__(self, hidden_dim: int, out_dim: int,
        ...
        if edge_score_type == 'gat':  # baseline
            self.convs = torch.nn.ModuleList(GATConv(hidden_dim,
                                                     hidden_dim) for _ in range(k_hops+c_hops))
        else:
            if k_hops > 0:
                self.k_convs = torch.nn.ModuleList([KCANKnowldgeConv(hidden_dim, hidden_dim,
                                                                     edge_score_type=edge_score_type,
                                                                     residual=self.residual,
                                                                     activation='leaky_relu') for _ in range(k_hops)])
            if c_hops > 0:
                self.c_convs = torch.nn.ModuleList([KCANConditionConv(hidden_dim, hidden_dim,
                                                                      residual=self.residual,
                                                                      activation='leaky_relu') for _ in range(c_hops)])
    ...
    def forward(self, subgraph: TorchSubGraphBatchData, x: torch.Tensor,
                e_w: torch.Tensor, e_b: torch.Tensor):
        alpha = None
        ...
        if self.edge_score_type == 'gat':
            ...
            if self.c_hops > 0:
                for i, conv in enumerate(self.c_convs):
                    x = conv(x, subgraph.adjs_t.edge_index, target_embeds, pre_alpha=alpha)
