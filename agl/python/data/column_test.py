...
from agl.python.data.column import AGLColumn, AGLDenseColumn, AGLRowColumn, AGLMultiDenseColumn
...
class ColumnTest(unittest.TestCase):
    ...
    def test_mutidense_column(self):
        ...
        multi_dense = AGLMultiDenseColumn(name="mdf", dim=3, dtype=np.int64, out_sep=" ", in_sep=",", concat=True)
        result = multi_dense(mock_multi_dense_int)
        self.assertEqual(result.dtype, torch.int64)
        truth = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
        self.assertListEqual(truth, result.numpy().tolist())

        multi_dense_int32 = AGLMultiDenseColumn(name="mdf", dim=3, dtype=np.int32, out_sep=" ", in_sep=",", concat=True)
        ...
        multi_dense_float = AGLMultiDenseColumn(name="mdf", dim=3, dtype=np.float32, out_sep=" ", in_sep=",", concat=True)
        ...
        multi_dense_double = AGLMultiDenseColumn(name="mdf", dim=3, dtype=np.float64, out_sep=" ", in_sep=",", concat=True)
