...
from agl.python.data.subgraph.pyg_inputs import TorchSubGraphBatchData, TorchEdgeIndex, TorchDenseFeature
...
def delete_root_index(subgraph: TorchSubGraphBatchData):
    ...
    edge_index = TorchEdgeIndex.create_from_tensor(edge_index[:, mask], size=subgraph.adjs_t.size, edge_indices=subgraph.adjs_t.edge_indices[mask])
    ...
    e_num = torch.Tensor([np.sum(i) for i in np.split(mask, cumsum_e_num)[:-1]], device=e_num.device).to(e_num.dtype)
    return TorchSubGraphBatchData.create_from_tensor(n_feats=subgraph.n_feats,
                                                     e_feats=e_fs,
                                                     y=subgraph.y,
                                                     adjs_t=edge_index,
...
class KCANMovielensModel(torch.nn.Module):
    def __init__(
        ...
        edge_score_type: str, residual: bool, k_hops: int, c_hops: int
    ):
        super().__init__()

        # initial layer
        self.node_embed_layer = torch.nn.Embedding(feats_dims['node_feature'], hidden_dim,
                                                   max_norm=5, scale_grad_by_freq=True)   # 点表征
        self.edge_embed_layer = torch.nn.Embedding(feats_dims['edge_feature'], hidden_dim,
                                                   max_norm=5, scale_grad_by_freq=True)  # 图谱模型中的边表征
        self.edge_embed_w_layer = torch.nn.Embedding(feats_dims['edge_feature'], hidden_dim,
                                                     max_norm=5, scale_grad_by_freq=True)  # 图谱中的边空间映射权重

        # encoder layer
        self._encoder = KCANEncoder(
            hidden_dim=hidden_dim, out_dim=out_dim,
            edge_score_type=edge_score_type, residual=residual, k_hops=k_hops, c_hops=c_hops
        )
    ...
    def forward(self, subgraph):
        x = self.node_embed_layer(subgraph.n_feats.features['node_feature']
                                  .to_dense().reshape([-1]))
        x = F.normalize(x, p=2, dim=-1)
        e_w = self.edge_embed_layer(subgraph.e_feats.features['edge_feature']
                                    .to_dense().reshape([-1]))
        e_w = F.normalize(e_w, p=2, dim=-1)
        e_b = self.edge_embed_w_layer(subgraph.e_feats.features['edge_feature']
                                      .to_dense().reshape([-1]))
...
my_collate = MultiGraphFeatureCollate(node_spec, edge_spec,
...
# step 4: 模型相关以及训练与测试
model = KCANMovielensModel(feats_dims={"node_feature": 188047, "edge_feature": 26},
                           hidden_dim=64, out_dim=1,
                           edge_score_type='transH', residual=True, k_hops=1, c_hops=1)
print(model)
...
for epoch in range(1, 101):
    ...
    if auc > best_auc:
        best_auc = auc
    t2 = time.time()
    print('Epoch: {:02d}, Loss: {:.4f}, auc: {:.4f}, best_auc: {:.4f}, train_time: {:4f}, val_time: {:4f}'
          .format(epoch, loss, auc, best_auc, t1-t0, t2-t1))
