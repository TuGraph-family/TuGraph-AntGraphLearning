...
def _parse_args():
    ...
    # input output args
    parser.add_argument("--input_node_table_name",
                        dest="input_node_table_name",
                        required=True,
                        type=str,
                        help="input node table name")
    parser.add_argument("--input_edge_table_name",
                        dest="input_edge_table_name",
                        required=True,
                        type=str,
                        help="input edge table name")
    parser.add_argument("--input_label_table_name",
                        dest="input_label_table_name",
                        default="",
                        type=str,
                        help="input label table name")
    parser.add_argument("--output_table_name_prefix",
                        dest="output_table_name_prefix",
                        required=True,
                        type=str,
                        help="output table name prefix")
    parser.add_argument("--jar_resource_path",
                        dest="jar_resource_path",
                        required=True,
                        type=str,
                        help="jar_resource_path")

    parser.add_argument("--neighbor_distance", dest="neighbor_distance", type=int, default=1, help="neighbor_distance")
    parser.add_argument("--index_metas",
                        dest="index_metas",
                        default="",
                        type=str,
                        help="index_metas")
    ...
    parser.add_argument("--sample_condition",
                        dest="sample_condition",
                        required=True,
                        type=str,
                        help="sample_condition")
    parser.add_argument("--subgraph_spec",
                        dest="subgraph_spec",
                        required=True,
                        type=str,
                        help="subgraph_spec")
...
def run(args):
    if not os.path.exists(args.jar_resource_path):
        ...
        subprocess.check_call('pwd && cd ../../../java && mvn clean package -Dmaven.test.skip', shell=True)
