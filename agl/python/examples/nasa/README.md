...
首先我们要把原始数据压缩成子图(pb string)的形式，使用如下命令可获取数据
```
wget http://alps-common.oss-cn-hangzhou-zmf.aliyuncs.com/part-00000-f1c59df8-da17-4c37-82ad-01f6b7f44089-c000.csv 
```

将得到的csv表 `part-00000-f1c59df8-da17-4c37-82ad-01f6b7f44089-c000.csv`，包含 'seed', 'graph_feature', 'node_id_list', 'label_list', 'flag_list' 三个字段，其中 'flag_list' 字段中以 'train', 'val', 'test' 标记训练集数据、验证集数据和测试集数据。然后，执行下述指令，转换flag_list的格式以便于后续处理：
```
sed -i 's/train/0/g' part-00000-f1c59df8-da17-4c37-82ad-01f6b7f44089-c000.csv
sed -i 's/\tval/\t1/g' part-00000-f1c59df8-da17-4c37-82ad-01f6b7f44089-c000.csv
sed -i 's/test/2/g' part-00000-f1c59df8-da17-4c37-82ad-01f6b7f44089-c000.csv
```
