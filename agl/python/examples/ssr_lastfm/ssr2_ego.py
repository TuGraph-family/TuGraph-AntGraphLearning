...
def get_features(filename, node_num):
    ...
    dataset = AGLTorchMapBasedDataset(data_filename,
                                      has_schema=True, column_sep=',',
                                      schema=["node_id","subgraph"])
...
class SSR2LastfmModel(torch.nn.Module):
    def __init__(self, hidden_dim: int, neg_sample_size: int=1, tau: float=0.2,
        ...
        self._encoder = SSR2Encoder(
            hidden_dim=hidden_dim, neg_sample_size=neg_sample_size,
            tau=tau, reg_infonce=reg_infonce,
            view_nums=view_nums
        )
...
model = SSR2LastfmModel(hidden_dim=64, neg_sample_size=3,
                        tau=0.5, reg_infonce=0.5)
print(model)
...
def train():
    ...
    for data in train_loader:
        ...
        t2 = time.time()
        if i % 100 == 0:
            print(f"batch {i}, loss:{pred_loss.item()}+{infonce_loss.item()}, time_cost:{t2-t1}")
    return total_loss / i
