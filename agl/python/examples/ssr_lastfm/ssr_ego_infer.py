...
# step 1: 构建dataset
node_file_name = "./data/agl_gzoo_bmdata_ssr_lastfm_open_source_node_table.csv"
edge_file_name = "./data/agl_gzoo_bmdata_ssr_lastfm_open_source_edge_table.csv"

# node data set
node_data_set = AGLTorchMapBasedDataset(node_file_name,
                                        has_schema=True, column_sep=',',
                                        schema=["node_id", "node_feature"])

# step 2: 构建collate function

# step 3: 构建 dataloader
# train loader
node_loader = DataLoader(dataset=node_data_set,
                         batch_size=512,
                         shuffle=False,
                         num_workers=3,
...
for data in node_loader:
    with torch.no_grad():
        node_feature = torch.LongTensor(np.array(data['node_feature'], dtype=np.int32)).to(device)
        x = model.node_embed_layer(node_feature.reshape([-1]))
        x = F.normalize(x, p=2, dim=-1)
        feats = x.cpu().numpy()
        ids = data['node_id']
        for id, feat in zip(ids, feats):
            print(int(id), " ".join(map(lambda x: str(round(x, 6)), feat)), file=f, sep=',')
f.close()
...
edge_loader = DataLoader(dataset=dataset,
                         batch_size=512,
                         shuffle=False)
model.eval()
...
for data in edge_loader:
    ...
    _, (edge_index, alpha) = model._encoder.convs.forward(x, edge_index, return_attention_weights=True)
    ...
    for d, a in zip(data, att):
        ...
        print(d[0], d[1], node1_type+'_'+node2_type,
              str(d[0])+'_'+str(d[1]), round(a[0], 6), file=fout, sep=',')
fout.close()
print('done')
