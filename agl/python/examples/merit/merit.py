...
class MERITModel(torch.nn.Module):
    def __init__(self, n_feat, e_feat, node_dim=None, d=None, fourier_basis=None, num_layers=3, 
                 n_head=4, null_idx=0, num_heads=1, drop_out=0.1, seq_len=None, log_max=None, 
                 kernel_size=3, context_type="lstm"):
        super().__init__()
        ...
        self._encoder = MERITEncoder(
                n_feat, e_feat, node_dim=node_dim, feat_dim=feat_dim, num_layers=num_layers, 
                seq_len=seq_len, n_head=n_head, drop_out=drop_out, d=d, 
                fourier_basis=fourier_basis, log_max=log_max, kernel_size=kernel_size,
