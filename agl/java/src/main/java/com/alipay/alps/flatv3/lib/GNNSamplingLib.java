...
public class GNNSamplingLib implements Serializable {
    ...
    public GNNSamplingLib(String subGraphSpec, String sampleCondition, int maxHop) throws Exception {
        ...
        for (int i = 0; i < maxHop; i++) {
            sampleConditions[i] = new SampleCondition(i < sampleCondStr.length ? sampleCondStr[i] : sampleCondStr[sampleCondStr.length - 1]);
        }
        this.maxHop = maxHop;
    }
                    ...
                    if (referColumns.containsKey(VariableSource.SEED)) {
                        for (String column : referColumns.get(VariableSource.SEED)) {
                            if (fieldFromLastHop.contains(column)) {
                                otherOutputSchema += ":index." + column + "." + getDtypeStr(subGraphSpec.getEdgeAttrs().get(column));
                            } else {
                                otherOutputSchema += ":seed." + column + "." + getDtypeStr(subGraphSpec.getSeedAttrs().get(column));
                            }
                        }
                    }
    ...
    public void processRootNodeOrder(String nodeId, String seed, int order, WriteSubGraphElement writer) {
        ...
        if (otherOutputLen == 0) {
            writer.write(seed, Constants.ROOT_INT, nodeId, String.format("%010d", order), nodeId, null, null);
        } else if (otherOutputLen == 1) {
            writer.write(seed, Constants.ROOT_INT, nodeId, String.format("%010d", order), nodeId, null, null, null);
        } else {
            writer.write(seed, Constants.ROOT_INT, nodeId, String.format("%010d", order), nodeId, null, null, null, null);
        }
    }
    ...
    public void processRootNode(String nodeIds, String seed, WriteSubGraphElement writer) {
        ...
        for (int i = 0; i < nodeIdArray.length; i++) {
            ...
            if (otherOutputLen == 0) {
                writer.write(seed, Constants.ROOT_INT, nodeId, String.format("%010d", i), nodeId, null, null);
            } else if (otherOutputLen == 1) {
                writer.write(seed, Constants.ROOT_INT, nodeId, String.format("%010d", i), nodeId, null, null, null);
            } else {
                writer.write(seed, Constants.ROOT_INT, nodeId, String.format("%010d", i), nodeId, null, null, null, null);
            }
        }
    }

    public void chooseNeighbor(String node1ID, List<String> node2IDs, List<String> edgeIDs, Map<String, BaseIndex> indexesMap,
        ...
        List<Integer> sampledNeighborIndex = neighborSelector.chooseNeighbor(indexesMap, neighborDataset, seedVariableMap);
        OtherOutput otherOutput = new OtherOutput.Builder()
        ...
        for (int i = 0; i < sampledNeighborIndex.size(); i++) {
            ...
            if (nodeOutputWidth == 0) {
                ...
                writer.write(seed, Constants.NODE_INT, null, null, node2ID, null, null, otherOutput.getNodeOtherOutput(0, neighborIdx));
                writer.write(seed, Constants.EDGE_INT, node1ID, node2ID, edgeID, null, null, null);
            } else if (nodeOutputWidth == 2) {
                writer.write(seed, Constants.NODE_INT, null, null, node2ID, null, null, otherOutput.getNodeOtherOutput(0, neighborIdx), otherOutput.getNodeOtherOutput(1, neighborIdx));
                writer.write(seed, Constants.EDGE_INT, node1ID, node2ID, edgeID, null, null, null, null);
            }
        }
    }

    public String generateGraphFeature(SubGraphElementIterator subGraphElementIterator, boolean removeEdgeAmongRoots) throws Exception {
        ...
        while (subGraphElementIterator.hasNext()) {
            ...
            if (entry.getInt(Constants.ENTRY_TYPE) == Constants.ROOT_INT) {
                rootIds.add(new ImmutablePair<>(Integer.valueOf(entry.getString(Constants.ENTRY_NODE2)), entry.getString(Constants.ENTRY_ID)));
                graphFeatureGenerator.addNodeInfo(entry.getString(Constants.ENTRY_ID), "default", entry.getString(Constants.ENTRY_FEATURE) == null ? "" : entry.getString(Constants.ENTRY_FEATURE));
            } else if (entry.getInt(Constants.ENTRY_TYPE) == Constants.NODE_INT) {
                graphFeatureGenerator.addNodeInfo(entry.getString(Constants.ENTRY_ID), "default", entry.getString(Constants.ENTRY_FEATURE) == null ? "" : entry.getString(Constants.ENTRY_FEATURE));
            } else if (entry.getInt(Constants.ENTRY_TYPE) == Constants.EDGE_INT) {
                edgeFeatures.add(new Tuple4<>(entry.getString(Constants.ENTRY_NODE1), entry.getString(Constants.ENTRY_NODE2), entry.getString(Constants.ENTRY_ID), entry.getString(Constants.ENTRY_FEATURE)));
            } else {
        ...
        try {
            ...
            for (Tuple4<String, String, String, String> edgeFeature : edgeFeatures) {
                if (removeEdgeAmongRoots && rootIdSet.contains(edgeFeature._1()) && rootIdSet.contains(edgeFeature._2()) && edgeFeature._1().compareTo(edgeFeature._2()) != 0) {
                    continue;
                }
            ...
            throw new RuntimeException("========failed for nodeIndices:" + Arrays.toString(graphFeatureGenerator.nodeIndices.entrySet().toArray())
