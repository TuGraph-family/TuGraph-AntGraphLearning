...
public class NeighborSelector {
     ...
     */
    public NeighborSelector(String otherOutputSchema, Filter filter, SampleCondition sampleCond) throws Exception {
     ...
     */
    public List<List<Integer>> process(Map<String, BaseIndex> indexesMap, HeteroDataset neighborDataset,
    ...
    public List<Integer> chooseNeighbor(Map<String, BaseIndex> indexesMap, HeteroDataset neighborDataset, Map<String, Element.Number> seedVariableMap) throws Exception {
        AbstractResult indexResult = null;
        if (filter == null) {
            List<RangeUnit> ranges = new ArrayList<>();
            ranges.add(new RangeUnit(0, neighborDataset.getArraySize() - 1));
