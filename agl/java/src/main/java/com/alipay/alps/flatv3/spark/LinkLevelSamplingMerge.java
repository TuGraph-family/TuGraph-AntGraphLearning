...
public class LinkLevelSamplingMerge extends NodeLevelSampling {
  ...
  public LinkLevelSamplingMerge(String edgeTable, String labelTable, String outputTablePrefix,
  ...
  public void runSamplingPipeline(SparkSession spark, Dataset<Row> linkDS, Dataset<Row> edgeDS,
    ...
    Dataset<Row> node1LinkDS = linkDS.withColumnRenamed("node1_id", "node_id")
        .select("node_id", "seed");
    Dataset<Row> node2LinkDS = linkDS.withColumnRenamed("node2_id", "node_id")
        .select("node_id", "seed");
    Dataset<Row> seedDS = node1LinkDS.union(node2LinkDS);
    Dataset<Row> nodesOrder = linkDS
        .withColumn("nodes_order", functions.concat_ws(" ", col("node1_id"), col("node2_id")))
        .select("nodes_order", "seed");
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      LinkLevelSamplingMerge gnn = new LinkLevelSamplingMerge(
