...
public class OtherOutput {
    ...
    public static class Builder {
            ...
            for (String part : parts) {
                ...
                switch (arrs[0]) {
                    case OUTPUT_NODE:
                        ...
                        parseOutputSchema(otherNodeOutputList, otherNodeOutputSource, otherNodeOutputType, otherNodeOutputField, part.split(":"));
                        break;
                    default:
                        throw new RuntimeException("schema is wrong :" + otherOutputSchema + ", we only support output other node attr.");
                }
            }
        ...
        private void parseOutputSchema(List[] otherOutputList, int[] otherOutputSource, String[] otherNodeOutputType, String[] otherNodeOutputField, String[] t) {
                    ...
                    case SOURCE_INDEX:
                        if (neighborDataset != null) {
                            if (neighborDataset.getAttributeList(ts[1]) == null) {
                                throw new RuntimeException("index output schema is wrong: " + (neighborDataset == null ? "neighborDataset is null" : "neighborDataset :" + neighborDataset + " missing key:" + ts[1]));
                            }
