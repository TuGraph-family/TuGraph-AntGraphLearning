...
public class HEGNN {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String hegnnMode = arguments.getOrDefault(Constants.HEGNN_MODE, Constants.HEGNN_PATH);
        ...
        Filter filter[] = new Filter[maxHop];
        String filterCond[] = arguments.get(Constants.FILTER_COND).split(";");
        for (int i = 0; i < maxHop; i++) {
            System.out.println("--------filterCond["+i+"] :" + (i < filterCond.length ? filterCond[i] : filterCond[filterCond.length-1]));
            filter[i] = new Filter(i < filterCond.length ? filterCond[i] : filterCond[filterCond.length-1]);
        }
        ...
        Dataset<Row> neighborDF = Utils.inputData(spark, inputEdge)
                ...
                .agg(
                    ...
                    collect_list("relation").as("collected_relation"));
        ...
        }, Encoders.bean(IndexInfo.class)).cache();
        indexDS.show();
        indexDS.printSchema();

        Dataset<SubGraphElement> resultGraphElementRoot = seedDS.map((MapFunction<Row, SubGraphElement>) row -> {
        ...
        Dataset<SubGraphElement> resultGraphElementHops[] = new Dataset[maxHop];
        for (int hop = 0; hop < maxHop; hop++) {
            Dataset<SubGraphElement> seedPropagationResults = indexDS.joinWith(seedAgg, indexDS.col("node1_id").equalTo(seedAgg.col("node_id")), "inner")
                    .flatMap(getPropergateFunction(otherOutputSchema, filter[hop], sampleCond[hop], hop), Encoders.bean(SubGraphElement.class));
            seedPropagationResults.show();
            seedPropagationResults.printSchema();
            resultGraphElementHops[hop] = seedPropagationResults;
            if (hop + 1 < maxHop) {
        ...
        Dataset<SubGraphElement> resultGraphElement = resultGraphElementRoot;
        if (hegnnMode.compareToIgnoreCase(Constants.HEGNN_PATH) == 0) {
            for (int hop = 0; hop < maxHop; hop++) {
                resultGraphElement = resultGraphElement.union(resultGraphElementHops[hop]);
            }
        } else {
            resultGraphElement = resultGraphElement.union(resultGraphElementHops[maxHop-1].filter("entryType = 'node'"));
        }
        ...
        Dataset<Row> subgraph = nodeFeatureDF.union(edgeFeatureDF).groupByKey((MapFunction<SubGraphElement, String>) row -> row.getSeed(), Encoders.STRING())
                .mapGroups(getGenerateGraphFeatureFunc(subGraphSpecs, hegnnMode), Encoders.tuple(Encoders.STRING(), Encoders.STRING())).toDF("seed", "graph_feature");
        subgraph.show();
        ...
        return
                new FlatMapFunction<Tuple2<IndexInfo, Row>, SubGraphElement>() {
                    ...
                    public Iterator<SubGraphElement> call(Tuple2<IndexInfo, Row> row) throws Exception {
                        ...
                        neighborAttrs.addAttributeList("relation", indexInfo.getRelation());
                        ...
                        for (int seedIdx = 0; seedIdx < seedList.size(); seedIdx++) {
                            ...
                            for (int i = 0; i < sampledNeighborIndex.size(); i++) {
                                ...
                                SubGraphElement subGraphNodeElement = new SubGraphElement(seed, "node", node2ID, node2ID, node2ID, null, null);
...
//                        }
    ...
    public static MapGroupsFunction<String, SubGraphElement, Tuple2<String, String>> getGenerateGraphFeatureFunc(SubGraphSpecs subGraphSpecs, String hegnnMode) {
        return new MapGroupsFunction<String, SubGraphElement, Tuple2<String, String>>() {
            ...
            public Tuple2<String, String> call(String key, Iterator<SubGraphElement> values) throws Exception {
                ...
                if (hegnnMode.compareToIgnoreCase(Constants.HEGNN_TWO_END) == 0) {
                    for (String nodeID : graphFeatureGenerator.nodeIndices.keySet()) {
                        edgeFeatures.add(new Tuple4<>(rootIds.get(0), nodeID, "e_"+nodeID, ""));
                    }
                }
                try {
