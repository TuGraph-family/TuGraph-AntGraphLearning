 ...
 * graphFeatureV2Generator.init();
 * graphFeatureV2Generator.addNodeInfo("1", "user", "0.2 1.1 2 3 4\t12");
 * graphFeatureV2Generator.addNodeInfo("2", "item", "0.2 1.1\t324");
 * graphFeatureV2Generator.addEdgeInfo("1", "2", "1-2", "buy", "2.2 3.4\t32435");
 * graphFeatureV2Generator.addNodeInfo("3", "item", "0.2 1.4\t324");
 * graphFeatureV2Generator.addEdgeInfo("2", "3", "2-3", "link", "2.2\t45");
 * String ret = graphFeatureGenerator.getGraphFeature(preId);
 */
public class GraphFeatureGenerator {
    ...
    public void addEdgeInfo(String node1Id, String node2Id, String edgeId, String edgeType, String features) {
        addEdgeInfo(node1Id, node2Id, edgeId, edgeType, subGraphSpecs.generateEdgeFeatures(features, edgeType));
    }

    public void addEdgeInfo(String node1Id, String node2Id, String edgeId, String edgeType, Features features) {
        try {
            if (!edgeIndices.containsKey(edgeId)) {
                edgeIndices.put(edgeId, edgeIndices.size());
                edgeTypes.add(edgeType);
                edgeNode1IndicesPerType.add(nodeIndicesPerType.get(nodeTypes.get(nodeIndices.get(node1Id))).get(node1Id));
                edgeNode2IndicesPerType.add(nodeIndicesPerType.get(nodeTypes.get(nodeIndices.get(node2Id))).get(node2Id));
    ...
    private GraphFeature.Builder generateGraphFeature(List<String> rootIds, Map<String, List<Integer>> rootTypeIndices) {
        ...
        for (String nodeType : allNodeTypes) {
            ...
            mergeIdsAndFeatures(mergedNodeIdBytes, mergedNodeFeatureBuilder, nodesOfType, nodeIndices, nodeFeatures);
        ...
        for (SubGraphSpec.EdgeSpec edgeSpec : allEdgeSpecList) {
            ...
            mergeIdsAndFeatures(mergedEdgeIdBytes, mergedEdgeFeatureBuilder, edgeIdsOfType, edgeIndices, edgeFeatures);
            // add merged ids to mergedEdgesBuilder
            mergedEdgesBuilder.setEids(IDs.newBuilder().setStr(BytesList.newBuilder().addAllValue(mergedEdgeIdBytes).build()));
    ...
    private void mergeIdsAndFeatures(List<ByteString> mergedIdBytes, Features.Builder mergedFeatureBuilder,
        ...
        try {
            ...
            for (String id : idsOfOneType) {
                ...
                for (String sparseKVKey : sparseKVFeatures.keySet()) {
                    ...
                    SparseKVFeatures.Builder sparseKVFeaturesBuilder = mergedSparseKVFeaturesBuilderMap.get(sparseKVKey);
                ...
                for (String sparseKKey : sparseKFeatures.keySet()) {
                    ...
                    SparseKFeatures.Builder sparseKFeaturesBuilder = mergedSparseKFeaturesBuilderMap.get(sparseKKey);
            ...
            for (String sparseKVKey : mergedSparseKVFeaturesBuilderMap.keySet()) {
                mergedSparseKVFeatureMap.put(sparseKVKey, mergedSparseKVFeaturesBuilderMap.get(sparseKVKey).build());
            }
            ...
            for (String sparseKKey : mergedSparseKFeaturesBuilderMap.keySet()) {
                mergedSparseKFeatureMap.put(sparseKKey, mergedSparseKFeaturesBuilderMap.get(sparseKKey).build());
            }
            mergedFeatureBuilder.putAllSpKs(mergedSparseKFeatureMap);
        } catch (Exception e) {
            throw new RuntimeException("error:" + Arrays.toString(e.getStackTrace())
                    + "\n input idsOfOneType:" + Arrays.toString(idsOfOneType.toArray()) + " id2Index:" + Arrays.toString(id2Index.entrySet().toArray()));
        }
    }

    private void appendDenseFeatures(DenseFeatures.Builder dstDenseFeaturesBuilder, DenseFeatures srcDenseFeatures) {
    ...
    private void appendSparseKFeatures(SparseKFeatures.Builder dstSparseKFeaturesBuilder, SparseKFeatures srcSparseKFeatures) {
    ...
    private void appendSparseKVFeatures(SparseKVFeatures.Builder dstSparseKVFeaturesBuilder, SparseKVFeatures srcSparseKVFeatures) {
