...
class EdgeUint {
 public:
  //EdgeUint(const std::string& name): edge_name_(name){}
  ...
  void AddSparseKArray(const std::string& f_name, std::shared_ptr<SparseKFeatureArray>& sf);

  void AddEdgeIndexCSR();

  std::shared_ptr<DenseFeatureArray> GetDenseFeatureArray(const std::string& f_name) const;
  std::shared_ptr<SparseKVFeatureArray> GetSparseKVArray(const std::string& f_name) const;
  std::shared_ptr<SparseKFeatureArray> GetSparseKArray(const std::string& f_name) const;

  std::shared_ptr<CSRAdj> GetCSRAdj();


 private:
  //std::string edge_name_; // 是否必要？
  // 如果 csr是排序好的，那么由csr 生成的 coo 一定是排好序的，因此，考虑只支持csr格式的外部数据
